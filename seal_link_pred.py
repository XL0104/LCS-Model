...
from models import SAGE, DGCNN, GCN, GIN,SiGraC
...
from utils import get_pos_neg_edges, extract_enclosing_subgraphs, construct_pyg_graph, do_edge_split, \
    Logger, AA, CN, HeatKernel_Chopper, calc_ratio_helper, do_seal_edge_split, n2v_emb, hk_chopper_subgraph
...
class SEALDataset(InMemoryDataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train',
        ...
        self.sparse_adj = SparseTensor(
            row=data.edge_index[0].to(self.device), col=data.edge_index[1].to(self.device),
    ...
    def process(self):
        ...
        L = D - A
        try:
            G_nx = nx.from_scipy_sparse_matrix(A)
        ...
        if not self.pairwise:
            print("Setting up Positive Subgraphs (HeatKernel + Chopper)")
            pos_list = extract_enclosing_subgraphs(
                pos_edge, A, L, G_nx, self.data.x, 1, self.node_label)
            print("Setting up Negative Subgraphs (HeatKernel + Chopper)")
            neg_list = extract_enclosing_subgraphs(
                neg_edge, A, L, G_nx, self.data.x, 0, self.node_label)
...
class SEALDynamicDataset(Dataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train',
        ...
        self.L = D - self.A
        try:
            self.G_nx = nx.from_scipy_sparse_matrix(self.A)
        except AttributeError:
            self.G_nx = nx.from_scipy_sparse_array(self.A)
...
def run_sweal(args, device):
    ...
    if args.dataset.startswith('ogbl'):
        ...
        data.edge_index = split_edge['train']['edge'].t()
        print(f"[DEBUG] After split and transpose, edge_index shape: {data.edge_index.shape}")
        ...
        dataset = DummyDataset(root=f'dataset/MiRNA_{args.dataset}')
        ...
        dataset = DummyDataset(root=f'dataset/KIBA_{args.dataset}')
        ...
        dataset = DummyDataset(root=f'dataset/PPI_{args.dataset}')
        ...
        print("Finish processing Custom dataset")
    ...
    if args.train_node_embedding:
        ...
        if args.dataset == 'Microbe':
            ...
            embedding_filename = 'embedding/embedding_miRNA.pt'
        elif args.dataset == 'KIBA':
            embedding_filename = 'embedding/embedding_kiba.pt'
        elif args.dataset == 'PPI':
            embedding_filename = 'embedding/embedding_ppi.pt'
        else:
        ...
        if args.use_concat:
            embedding = torch.load(embedding_filename, map_location='cpu')
            if args.use_feature:
                emb = torch.cat([data.x, embedding], 1)
            else:
                emb = embedding
            emb = emb.to(device)
    else:
    ...
    for run in range(args.runs):
        ...
        elif args.model == 'SiGraC':
            model = SiGraC(hidden_channels=args.hidden_channels,num_layers=args.num_layers,max_z=max_z,train_dataset=train_dataset,
                           use_feature=args.use_feature,node_embedding=emb,dropout=args.dropout,dropedge=args.dropedge).to(device)

        elif args.model == 'MVGCN':
            model = MVGCN(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fast_split', action='store_true', default=False,
    ...
    parser.add_argument('--use_n2v', action='store_true', default=True)
    parser.add_argument('--use_retrofitting', action='store_true', default=True)
    parser.add_argument('--use_concat', action='store_true', default=True)
    # Training settings
    ...
    parser.add_argument('--use_heuristic', type=str, default=None,
                        help="test a link prediction heuristic (CN, AA or HeatKernel_Chopper)")
    parser.add_argument('--dataset_stats', action='store_true',
                        help="Print dataset statistics")
    parser.add_argument('--m', type=int, default=0, help="Set rw length") #3
    parser.add_argument('--M', type=int, default=0, help="Set number of rw") #20
    ...
    parser.add_argument('--train_gae', action='store_true', default=False, help="Train GAE on the dataset")
